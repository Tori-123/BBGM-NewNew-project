...
主故事：学生注册/登录 → `POST /api/v1/posts` 发帖（含活动字段）→ 未登录会话在首页与栏目读到同一条。依据 `docs/PRD.md`、`docs/schema.md`、`docs/UI_BLUEPRINT.md`。
...
- [x] `GET /api/v1/posts`：公开。Query `category?` `page`（默认 1）`page_size`（默认 20，最大 50）。无 category 按 `created_at` 降序；有则只返回该枚举。`200` `{items, page, page_size, total}`；`items[]` 为 PostSummary（含 `is_activity` `starts_at` `location` `excerpt` `author`）。非法分页 `400 bad_request`；非法 category `422`；读失败 `503`
- [x] `GET /api/v1/posts/{post_id}`：公开。合法 UUID 且存在则 `200` PostDetail（含 `body`）；非 UUID `400`；不存在 `404 not_found`；读失败 `503`
- [x] `POST /api/v1/posts`：需会话。校验 `title` `body` `category` `is_activity`；`is_activity=true` 时 `starts_at`（ISO UTC）与 `location` 必填，`false` 时两者为 null。作者取当前用户，忽略客户端 `author_id`/`status`。成功 `201` PostDetail，`status=published`，生成 `excerpt`（body 截断 ≤160），**同一事务**写 `audit_events`（不出现在响应）。无会话 `401`；校验失败 `422` 且不落 `published` 行；帖或审计写入失败整笔回滚 `503`，不返回 201
...
- [x] 增加路由（如 React Router）：蓝图中的 `/` `/news` `/sports` `/dorm-life` `/events` `/opinion` `/photo` `/posts/:postId` `/submit` `/me/posts` `/sign-in` `/register` `/about` `/contact`
- [x] `PaperShell`：顶栏 ABOUT/CONTACT、账号区、本地 `Today:`；报头搜索壳 + `BBGM | SCOOP` + Student Media；标语；导航 NEWS…SUBMIT。字体 Playfair Display + Inter；色 `#1A4FBF`、图框 `#D6DEEE`、黑线
- [x] 搜索提交：报头下静态 “Search comes in a later version.”（不调检索 API）
- [x] 访客：`SIGN IN`。已登录 Mock：`display_name`、`MY POSTS`、`SIGN OUT`
- [x] 首页按蓝图槽位填 Mock `GET /posts` 的 `items[0]`…`[15]`，缺槽空态不循环灌数。Upcoming：同一 `items` 里 `is_activity===true` 的 `title` `starts_at` `location`。Photo/Track/Art 静态空壳
- [x] 栏目页：`category` 请求对应枚举；行绑 `title` `excerpt` `category`；活动行加 `starts_at` `location`。空态、加载横线、错误条（用 Mock 错误 JSON）
- [x] `/opinion` `/photo`：固定空态，**不请求**非法 category
- [x] 详情：`category` `title` `author.display_name` `created_at` `body`；活动加 `starts_at` `location`。Mock 404 显示 “Story not found.”
- [x] `/submit`：左 `title` `body`，右 `category`（四枚举）`is_activity` 及条件 `starts_at` `location`，`Publish`。未登录 Mock 跳 `/sign-in?next=/submit`。422 写在 `error.fields[]` 对应输入下
...
- [x] 首页/栏目/详情改真实 GET；`SUBMIT` 已登录去 `/submit`，未登录 `/sign-in?next=/submit`
...
- [x] 浏览器验证：注册 Jordan 类账号 → 登录 → 发宿舍帖 → 无痕窗口打开首页与 `/dorm-life` 能看到标题与详情正文
- [x] 再发一条 `is_activity=true` 的 sports/events 帖，栏目列表与详情可见 `starts_at` `location`，首页 Upcoming 也能看到
- [x] 错误凭据无法进入发帖页；未登录 POST 被拒；活动缺地点停留在 `/submit` 且库无新帖
...
| 2 | 登录后可进发帖页 | `GET /me` + `/submit` | [x] |
| 3 | 发帖得详情 URL，`published`，作者为当前用户 | `POST /posts` | [x] |
| 4 | 另一未登录会话在首页/栏目/详情看到同一帖 | 公开 GET | [x] |
| 5 | 活动帖列表与详情展示时间地点 | `is_activity` 字段 | [x] |
| 6 | 重启后帖与账号仍在 | 持久化 | [x] |
